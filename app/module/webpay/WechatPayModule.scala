...
object WechatPayModule {
    def prepayid(data : JsValue) : JsValue = 
        try {
            ...
            val spbill_create_ip = (data \ "spbill_create_ip").asOpt[String].map (x => x).getOrElse(WechatSercurityFunc.spbill_create_ip)
        		...
        		val valxml = """<xml><appid>%s</appid><body><![CDATA[%s]]></body><mch_id>%s</mch_id><nonce_str>%s</nonce_str><notify_url>%s</notify_url><out_trade_no>%s</out_trade_no><spbill_create_ip>127.0.0.1</spbill_create_ip><total_fee>%d</total_fee><trade_type>%s</trade_type><sign><![CDATA[%s]]></sign></xml>"""
        		  			.format(WechatSercurityFunc.app_id, pay_body, WechatSercurityFunc.mch_id, pay_noncestr, WechatSercurityFunc.fake_notify_url, trade_no, fee, str_md5)
